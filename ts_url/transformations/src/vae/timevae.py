...
class TrendLayer(nn.Module):
    def __init__(self, seq_len, feat_dim, output_dims, trend_poly):
        super(TrendLayer, self).__init__()
        self.seq_len = seq_len
        self.feat_dim = feat_dim
        self.output_dims = output_dims
        self.trend_poly = trend_poly
        self.trend_dense1 = nn.Linear(self.output_dims, self.feat_dim * self.trend_poly)
...
class SeasonalLayer(nn.Module):
    def __init__(self, seq_len, feat_dim, output_dims, custom_seas):
        ...
        self.dense_layers = nn.ModuleList([
            nn.Linear(output_dims, feat_dim * num_seasons)
...
class LevelModel(nn.Module):
    def __init__(self, output_dims, feat_dim, seq_len):
        super(LevelModel, self).__init__()
        self.output_dims = output_dims
        self.feat_dim = feat_dim
        self.seq_len = seq_len
        self.level_dense1 = nn.Linear(self.output_dims, self.feat_dim)
...
class TimeVAEDecoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, output_dims, trend_poly=0, custom_seas=None, use_residual_conn=True, encoder_last_dense_dim=None):
        ...
        self.level_model = LevelModel(self.output_dims, self.feat_dim, self.seq_len)

        if use_residual_conn:
            self.residual_conn = ResidualConnection(seq_len, feat_dim, hidden_layer_sizes, output_dims, encoder_last_dense_dim)

    def forward(self, z):
        outputs = self.level_model(z)
        if self.trend_poly is not None and self.trend_poly > 0:
            trend_vals = TrendLayer(self.seq_len, self.feat_dim, self.output_dims, self.trend_poly)(z)
            outputs += trend_vals

        # custom seasons
        if self.custom_seas is not None and len(self.custom_seas) > 0:
            cust_seas_vals = SeasonalLayer(self.seq_len, self.feat_dim, self.output_dims, self.custom_seas)(z)
...
class TimeVAE(BaseVariationalAutoencoder):
    ...
    def _get_encoder(self):
        encoder = TimeVAEEncoder(self.seq_len, self.feat_dim, self.hidden_layer_sizes, self.output_dims)
        return encoder

    def _get_decoder(self):
        return TimeVAEDecoder(self.seq_len, self.feat_dim, self.hidden_layer_sizes, self.output_dims, self.trend_poly, self.custom_seas, self.use_residual_conn, self.encoder.encoder_last_dense_dim)
