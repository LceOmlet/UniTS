...
class TNCDataset(Dataset):
    def __init__(self, data, optim_config, epsilon=3, label=None, adf=False, **kwargs):
        ...
        self.sliding_gap = int(self.window_size * 25.2)
        self.window_per_sample = (self.T - 2 * self.window_size) // self.sliding_gap
        self.mc_sample_size = optim_config.get("mc_sample_size")
        self.state = label
        self.augmentation = optim_config.get("augmentation")
        self.adf = adf
        if not self.adf:
            self.epsilon = epsilon
            self.delta = 5 * self.window_size * epsilon
