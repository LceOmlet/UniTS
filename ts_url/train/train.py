...
from ..models.tnc.tnc import TNCDataset, Discriminator
...
def step_tnc(model, batch, loss_module, optimizer, optim_config, device, **kwargs):
    ...
    x_t = batch["x_t"]  # (B, F, L)
    x_p = batch["x_p"]  # (B, mc, F, L)
    x_n = batch["x_n"]  # (B, mc, F, L)
    ...
    neighbors = torch.ones_like(d_p).to(device)
    non_neighbors = torch.zeros_like(d_n).to(device)
