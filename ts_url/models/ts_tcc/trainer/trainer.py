...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    ...
    for epoch in range(1, config.num_epoch + 1):
        ...
        logger.debug(f'\nEpoch : {epoch}\n'
                     ...
                     f'NMI     : {valid_loss:.4f}\t | \tRI     : {valid_acc:2.4f}')
...
def model_evaluate(model, temporal_contr_model, test_dl, device, training_mode):
    ...
    with torch.no_grad():
        for data, labels, _, _ in test_dl:
            ...
            features = temporal_contr_model.seq_transformer(features)
            features_test.append(features.cpu().numpy())
            labels_test.append(labels.cpu().numpy())
        test_repr = features.cpu().numpy() if len(features_test) == 1 else np.vstack(features_test)
        labels = labels.cpu().numpy() if len(labels_test) == 1 else np.hstack(labels_test)
        label_num = np.max(labels) + 1
        pca = PCA(10)
        new_test_repr = pca.fit_transform(test_repr)
        kmeans = KMeans(label_num)
        pred = kmeans.fit_predict(new_test_repr)
        

        NMI_score = normalized_mutual_info_score(labels, pred)
        RI_score = rand_score(labels, pred)
    return NMI_score, RI_score , [], []
