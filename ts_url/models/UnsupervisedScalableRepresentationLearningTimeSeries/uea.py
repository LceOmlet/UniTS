...
def load_UEA_dataset(path, dataset):
    ...
    train_labels = numpy.empty(train_size, dtype=int)
    test_labels = numpy.empty(test_size, dtype=int)
...
if __name__ == '__main__':
    ...
    ans = classifier.score(test, test_labels)
    with open(str(args.rank) + ".txt", "w") as res:
        res.write(json.dumps(ans) + "\n")
    print("Test accuracy: " + str(ans))
