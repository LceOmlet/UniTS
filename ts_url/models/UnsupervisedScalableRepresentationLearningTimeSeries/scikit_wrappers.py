...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def score(self, X, y, batch_size=50):
        ...
        """
        features = self.encode(X, batch_size=batch_size)
        label_num = np.max(y) + 1
        pca = PCA(10)
        new_test_repr = pca.fit_transform(features)
        kmeans = KMeans(label_num)
        pred = kmeans.fit_predict(new_test_repr)
        

        NMI_score = normalized_mutual_info_score(y, pred)
        RI_score = rand_score(y, pred)
        return {"NMI":NMI_score, "RI": RI_score, "acc": self.classifier.score(features, y)}
