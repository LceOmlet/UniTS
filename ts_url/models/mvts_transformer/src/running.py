...
reprs = None
def get_rep(module, input, output):
    global reprs
    reprs = input[0].cpu().numpy()
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        epoch_loss = 0  # total loss of epoch
        total_active_elements = 0  # total unmasked elements in epoch
        handle = self.model.output_layer.register_forward_hook(get_rep)

        if keep_all:
            per_batch = {'target_masks': [], 'targets': [], 'predictions': [], 'metrics': [], 'IDs': [], 'reps': []}
        for i, batch in enumerate(self.dataloader):
        ...
        if keep_all:
            return epoch_metrics, per_batch
        else:
            return epoch_metrics
