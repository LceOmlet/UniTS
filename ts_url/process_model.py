...
from .models.tnc.models import RnnEncoder, WFEncoder
from .models.tnc.tnc import Discriminator
...
class TNC(nn.Module):
    def __init__(self, feat_dim, device, hidden_size=100, encoding_size=10, encoder_type="rnn", **kwargs):
        ...
        elif self.encoder_type == "wf":
            self.encoder = WFEncoder(
                encoding_size=encoding_size
            )
        else:
            raise ValueError(f"Unsupported encoder type: {encoder_type}")
        self.disc_model = Discriminator(self.encoder.encoding_size, device)
